...
struct ContentView: View {
    ...
    private var patchOptions: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(columns: [GridItem(.flexible(), spacing: 12), GridItem(.flexible(), spacing: 12)], spacing: 12) {
                patchCard(name: "Aim Drag", target: "FREE FIRE • NORMAL", package: "x1.3105", color: AppTheme.accent, state: $aimDragEnabled)
                patchCard(name: "Aim Neck", target: "FREE FIRE • NORMAL", package: "x2.3105", color: AppTheme.secondaryAccent, state: $aimNeckEnabled)
                patchCard(name: "Antenna", target: "FREE FIRE • NORMAL", package: "x3.3105", color: AppTheme.secondaryAccent, state: $hspeitoffEnabled)
                patchCard(name: "144 FPS", target: "FREE FIRE • NORMAL", package: "x4.3105", color: AppTheme.secondaryAccent, state: $hyperBalamagicaEnabled)
                patchCard(name: "Aim Body", target: "FREE FIRE • NORMAL", package: "x5.3105", color: AppTheme.accent, state: $aimBodyPackageEnabled)
                patchCard(name: "Aim Chest", target: "FREE FIRE • NORMAL", package: "x6.3105", color: AppTheme.secondaryAccent, state: $aimChestPackageEnabled)
                patchCard(name: "Magic", target: "FREE FIRE • NORMAL", package: "x7.3105", color: AppTheme.accent, state: $magicEnabled)
            }
    ...
    private func syncPatchStates() {
        aimDragEnabled = isPatchActive("x1.3105")
        aimNeckEnabled = isPatchActive("x2.3105")
        hspeitoffEnabled = isPatchActive("x3.3105")
        hyperBalamagicaEnabled = isPatchActive("x4.3105")
        aimBodyPackageEnabled = isPatchActive("x4.3105")
        aimChestPackageEnabled = isPatchActive("x5.3105")
        magicEnabled = isPatchActive("x6.3105")
    }
    ...
    private func setPatchState(for packageFilename: String, enabled: Bool) {
        ...
        case "x1.3105": aimDragEnabled = enabled
        case "x2.3105": aimNeckEnabled = enabled
        case "x3.3105": hspeitoffEnabled = enabled
        case "x4.3105": hyperBalamagicaEnabled = enabled
        case "x5": aimBodyPackageEnabled = enabled
        case "x6.3105": aimChestPackageEnabled = enabled
        case "x7.3105": magicEnabled = enabled
        default: break
        }
    }
